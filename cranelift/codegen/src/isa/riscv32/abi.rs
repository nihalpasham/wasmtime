...
use crate::isa::riscv32::{inst::*, Riscv32Backend};
...
/// Support for the Riscv32 ABI from the caller side (at a callsite).
pub(crate) type Riscv32ABICallSite = CallSite<Riscv32MachineDeps>;
...
impl Riscv32ABICallSite {
    pub fn emit_return_call(
        mut self,
        ctx: &mut Lower<Inst>,
        args: isle::ValueSlice,
        _backend: &Riscv32Backend,
    ) {
        ...
        match dest {
            ...
            CallDest::ExtName(name, RelocDistance::Far) => {
                let callee = ctx.alloc_tmp(ir::types::I32).only_reg().unwrap();
